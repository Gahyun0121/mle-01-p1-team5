...
`gpt-4o-mini` / `temperature=0`. 시스템 메시지에 대상 국가를 명시하고 8개 규칙을 둡니다.
...
| 나미비아 누락 | 국가 필터 단계의 결측 처리 문제로 실제 대상은 118개국이어야 합니다. → [트러블슈팅 #2](#8-트러블슈팅) |
...
| 전역 공지 미검색 | 국가에 속하지 않는 `ALL` 공지가 인덱스에 있지만, 검색 시 국가명으로 필터링해 도달할 수 없습니다. |
...
**배운 것** — 안전정보 서비스에서는 있는 정보를 놓치는 쪽이 더 위험하기 때문에 도메인에 따라 프롬프트의 엄격도를 다르게 잡아야 하는 것을 알게 되었습니다.
