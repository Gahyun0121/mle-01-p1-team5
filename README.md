...
| travel_destinations.json | Kaggle travel_destinations | 111행 | 43 | 추천 |
...
- **챗봇**은 안전공지·사건사고 본문을 인덱싱해 117개국 범위입니다.
...
- 모든 데이터셋을 ISO 코드로 연결 — 한국어 국가명은 표기가 흔들려 조인 실패
  (`Turkey → 튀르키예`, `United States of America → 미국` 수동 보정)
...
| 나미비아 누락 | 국가 필터 스크립트(`filter_common_countries.py`)에 `keep_default_na=False`가 누락돼 ISO2 코드 `NA`가 결측 처리됐습니다. 실제 대상은 118개국이어야 하며, 벡터 DB 재구축 비용을 고려해 현 사이클에서는 수정하지 않았습니다. |
| 코소보 제외 | ISO 3166 정식 코드가 없어(임시 코드 `XK`) 대상에서 제외됐습니다. |
| 전역 공지 미검색 | 특정 국가에 속하지 않는 전체 공지 5건은 국가 단위 필터링 구조상 검색되지 않습니다. |
